...
class RewardPointsRefund
{
     ...
     * Event manager
     *
     * @var \Coditron\Reward\Model\Reward\Refund\SalesRuleRefund
     */
    protected $salesRuleRefund;

    /**
     * @param \Coditron\Reward\Model\ResourceModel\Reward\History\CollectionFactory $historyCollectionFactory
     ...
     * @param \Coditron\Reward\Model\Reward\Refund\SalesRuleRefund $salesRuleRefund
     * @codeCoverageIgnore
     */
    public function __construct(
        \Coditron\Reward\Model\ResourceModel\Reward\History\CollectionFactory $historyCollectionFactory,
        \Magento\Store\Model\StoreManagerInterface $storeManager,
        \Magento\Framework\Event\ManagerInterface $eventManager,
        \Coditron\Reward\Model\RewardFactory $rewardFactory,
        \Coditron\Reward\Helper\Data $rewardData,
        \Coditron\Reward\Model\Reward\Refund\SalesRuleRefund $salesRuleRefund
    ) {
        ...
        if ($creditmemo->getBaseRewardCurrencyAmount() && $isRefundAllowed) {
            if ((int)$creditmemo->getRewardPointsBalanceRefund() > 0) {
                $this->getRewardModel()
                    ...
                    ->setAction(\Coditron\Reward\Model\Reward::REWARD_ACTION_CREDITMEMO)
    ...
    protected function updateHistoryRow(\Magento\Sales\Model\Order\Creditmemo $creditmemo)
    {
        ...
        if ($this->rewardData->getGeneralConfig('deduct_automatically')) {
            $reward->setPointsDelta(-$rewardPointsAmountToVoid)
                ->setAction(\Coditron\Reward\Model\Reward::REWARD_ACTION_CREDITMEMO_VOID)
                ->setActionEntity($order)
                ->updateRewardPoints();
     ...
     * @return \Coditron\Reward\Model\Reward\History|null
     */
    protected function getRewardHistoryRecordForOrder(\Magento\Sales\Model\Order $order)
    {
        $rewardHistoryCollection = $this->historyCollectionFactory->create()
            ...
            ->addFilter('main_table.action', \Coditron\Reward\Model\Reward::REWARD_ACTION_ORDER_EXTRA);
