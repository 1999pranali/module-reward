...
namespace Coditron\Reward\Model\ResourceModel\Reward;

use Coditron\Reward\Model\Reward\History as ModelRewardHistory;
...
class History extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('coditron_reward_history', 'history_id');
    ...
    public function isExistHistoryUpdate($customerId, $action, $websiteId, $entity)
    {
        $select = $this->getConnection()->select()->from(
            ['reward_table' => $this->getTable('coditron_reward')],
            []
        )->joinInner(
    ...
    public function expirePoints($websiteId, $expiryType, $limit)
    {
        ...
        if (count($expiredHistoryIds) > 0) {
            ...
            foreach ($expiredAmounts as $rewardId => $expired) {
                ...
                $connection->update($this->getTable('coditron_reward'), $bind, $where);
            }
